...
class SensorIntegrationConfigurator(IntegrationConfigurator):
    def __init__(self, prefix, ip, entry):
        IntegrationConfigurator.__init__(self, prefix, ip, entry)

    def configure(self, config):
        super(SensorIntegrationConfigurator, self).configure(config)
        ...
        #     "value_template":"{{% if value_json.valve | float > 0 %}}ON{{% else %}}OFF{{% endif %}}",
        #     #"device": device
        #     }
        # payload_template_json = "{" + json.dumps(payload_template) + "}"
        # print("payload_template_json: ", payload_template_json)
